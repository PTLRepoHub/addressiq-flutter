#!/usr/bin/env bash
# Regenerates lib/src/generated/build_config.dart from the environment.
#
...
# a local concern and stays a literal in lib/src/api/environment.dart.
#
...
# "unset — keeping the checked-in default" — which meant a misconfigured release
...
// \`development\`.
const String kStagingApiUrl = '$V_STAGING_ADDRESSIQ_API_BASE_URL';
const String kStagingIngestUrl = '$V_STAGING_ADDRESSIQ_INGEST_BASE_URL';
const String kStagingCdnUrl = '$V_STAGING_ADDRESSIQ_CDN_BASE_URL';

const String kProdApiUrl = '$V_PROD_ADDRESSIQ_API_BASE_URL';
const String kProdIngestUrl = '$V_PROD_ADDRESSIQ_INGEST_BASE_URL';
const String kProdCdnUrl = '$V_PROD_ADDRESSIQ_CDN_BASE_URL';

/// Version of the widget published to the CDN, WITHOUT the leading \`v\`
/// (e.g. \`0.4.0\`). Baked from the \`.widget-version\` file; \`''\` when absent.
const String kWidgetVersion = '$WIDGET_VERSION';
