...
# release cut by hand (`chore(release): X`) tags the commit, bumps pubspec and
# writes the CHANGELOG, but leaves the manifest behind — after which
...
shipped="$(sed -n 's/^version: *//p' pubspec.yaml | head -1 | tr -d ' \t\r\n')"

if [ "$manifest" != "$shipped" ]; then
  echo "::error::release-please manifest says '$manifest' but pubspec.yaml says '$shipped'." >&2
...
echo "release-please manifest and pubspec.yaml agree: $shipped"
